...
static void press_equal(void) {
    if (!op) return;
    double cur=display_to_double(); double res=acc;
    if (op=='+') res=acc+cur; else if (op=='-') res=acc-cur;
    else if (op=='*') res=acc*cur; else if (op=='/') res=(cur==0.0)? NAN : acc/cur;
    set_display_double(res); acc=res; op=0; entering_second=0; has_dot=(strchr(display,'.')!=NULL);
}
...
static void draw_ui(void){
    ...
    char buf[40]; snprintf(buf,sizeof(buf),"%29s",display);
    printf("│%s│", buf);
    ...
    for(r=0;r<5;++r){
        for(c=0;c<4;++c){
            int x=2+c*9; int y=y0+r*3; const char* label=rows[r][c];
            int selected=(r==sel_r && c==sel_c);
            pspDebugScreenSetXY(x,y);
            if(selected) printf("[ %-3s ]", label);
...
static void press_button(const char* label){
    ...
    if(!strcmp(label,"+/-")){ toggle_sign(); return; }
    if(!strcmp(label,"←")){ backspace(); return; }
    if(!strcmp(label,"=")){ press_equal(); return; }
    if(!strcmp(label,"%")){ double v=display_to_double(); v/=100.0; set_display_double(v); return; }
    if(!strcmp(label,".")){ input_digit('.'); return; }
    if(!strcmp(label,"+")||!strcmp(label,"-")||!strcmp(label,"*")||!strcmp(label,"/")){
